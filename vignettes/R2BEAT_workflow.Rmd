...
inp <- prepareInputToAllocation2(
        ...
        stratum,     # strata variables
        target,      # target variables
        deff_level,  # deff variables
...
```{r}
newstrata <- check_input(strata=inp$file_strata,
                         des=inp$des_file,
...
alloc <- beat.2st(file_strata = inp$file_strata, 
                  errors = cv, 
...
```{r}
set.seed(1234)
sample_1st <- select_PSU(alloc, type="OPT", pps=TRUE, plot=TRUE)
```
...
```{r}
nrow(samp)
sum(alloc$alloc$OPT[-nrow(alloc$alloc)])
```
