...
```{r , warning=FALSE}
a <- beat.1cv(stratif=strata,alloc=equal$alloc$OPT)
b <- beat.1cv(strata,intermediate2$alloc$OPT)
c <- beat.1cv(strata,final$alloc$OPT)
row.names(a) <- NULL
tot <- cbind(a$expectedCV[,c(1,2,4)],b$expectedCV[,4],c$expectedCV[,4])
...
dom2$Group.1 <- "DOM2"
dom3$Group.1 <- "DOM3"
tot1 <- merge(tot,dom1,by.x="DOM",by.y="Group.1")
tot2 <- merge(tot,dom2,by.x="DOM",by.y="Group.1")
tot3 <- merge(tot,dom3,by.x="DOM",by.y="Group.1")
